...
# Qwen3.5 35B A3B P/D Nightly Deployment

## 1. 文档概述

Qwen3.5-35B-A3B P/D deployment with MTP decode graph, reasoning, and tool-call validation.

- Case: `qwen35-35b-a3b-pd-nightly`
- Level: `nightly`
- Owner: `vllm-ascend-ci`
- Audience: vLLM Ascend deployment engineers
- Difficulty: advanced
- Tags: nightly, benchmark, pd, qwen35, mtp, reasoning, tool-call
...
- `extra`: MooncakeLayerwiseConnector, Qwen3.5 MTP, reasoning parser, and qwen3_xml tool parser are required.

## 3. 模型信息

- `name`: Qwen/Qwen3.5-35B-A3B
- `source`: Local mirror
- `path_hint`: Set MODEL_ROOT to the directory containing Qwen3.5-35B-A3B-w8a8-mtp.

## 4. 部署拓扑

- Service `qwen35-35b-a3b-proxy` runs as `command` on `0.0.0.0:8230` with role `proxy` and card_count=`0`.
- Service `qwen35-35b-a3b-prefill` runs as `vllm-serve` on `0.0.0.0:8231` with role `prefill` and card_count=`8`.
- Service `qwen35-35b-a3b-decode` runs as `vllm-serve` on `0.0.0.0:8232` with role `decode` and card_count=`8`.
...
export PREFILL_EXTRA_PORT_0=36331
export DECODE_EXTRA_PORT_0=36332
export PREFILL_SERVERS=0.0.0.0:8231
export DECODE_SERVERS=0.0.0.0:8232
```
...
```bash
# qwen35-35b-a3b-proxy (proxy)
python3 .ci/scripts/pd_proxy.py --host 0.0.0.0 --port 8230

# qwen35-35b-a3b-prefill (prefill)
vllm serve Qwen/Qwen3.5-35B-A3B --served-model-name qwen35-35b-a3b-pd-nightly --host 0.0.0.0 --port 8231 --tensor-parallel-size 8 --enable-expert-parallel --quantization ascend --trust-remote-code --no-enable-prefix-caching --reasoning-parser qwen3 --enable-auto-tool-choice --tool-call-parser qwen3_xml

# qwen35-35b-a3b-decode (decode)
vllm serve Qwen/Qwen3.5-35B-A3B --served-model-name qwen35-35b-a3b-pd-nightly --host 0.0.0.0 --port 8232 --tensor-parallel-size 2 --data-parallel-size 4 --enable-expert-parallel --quantization ascend --trust-remote-code --no-enable-prefix-caching --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY","cudagraph_capture_sizes":[4]}' --speculative-config '{"method":"qwen3_5_mtp","num_speculative_tokens":3}' --reasoning-parser qwen3 --enable-auto-tool-choice --tool-call-parser qwen3_xml
```
...
local_model_path: ${MODEL_ROOT}/Qwen3.5-35B-A3B-w8a8-mtp
served_model_name: qwen35-35b-a3b-pd-nightly
args:
- --served-model-name
- qwen35-35b-a3b-pd-nightly
- --host
- 0.0.0.0
- --port
- '8231'
...
```bash
curl -fsS http://0.0.0.0:8230/health
```

```bash
curl -sS -X POST http://0.0.0.0:8230/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{
  "model": "qwen35-35b-a3b-pd-nightly",
  "messages": [
    {
      "role": "user",
...
```bash
vllm bench serve --model qwen35-35b-a3b-pd-nightly --host 127.0.0.1 --port 8230 --dataset-name random --random-input-len 200 --random-output-len 128 --num-prompts 32 --request-rate 1 --save-result --result-dir reports/nightly/benchmark/qwen35-35b-a3b-pd-nightly
```
...
| Parameter | Value |
| --- | --- |
| `--served-model-name` | `qwen35-35b-a3b-pd-nightly` |
| `--host` | `0.0.0.0` |
| `--port` | `8231` |
