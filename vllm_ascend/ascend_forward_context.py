...
def select_moe_comm_method(num_tokens: int, vllm_config: VllmConfig, is_draft_model=False) -> MoECommType | None:
    ...
    4. On A3 with expert parallel, prefer fused MC2 when the configured fused
       operator path is available for the current execution; otherwise use MC2
       within capacity or all-to-all.
    5. On 310P, always use all-gather.

    Args:
        num_tokens (int): The number of tokens in the current batch.
        vllm_config (VllmConfig): Runtime configuration for the model.
        is_draft_model (bool): Whether the current execution is for the
            draft/MTP model. Kept for caller compatibility.
